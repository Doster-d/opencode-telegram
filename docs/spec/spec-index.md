# Spec Index

Canonical project documentation lives in `docs/spec`.

## Start Here

- [Getting Started](runbooks/getting-started.md)
- [Configuration](runbooks/configuration.md)
- [Local Development Runbook](runbooks/local-dev.md)

## Development

- [Go Development Workflow](runbooks/development-workflow.md)
- [Testing, Linting, Coverage](runbooks/testing-quality.md)
- [Release Checklist](runbooks/release-checklist.md)

## Product and System

- [Telegram Command Specification](features/telegram-commands.md)
- [Telegram UX Session/Run Isolation](features/telegram-ux-session-run-isolation.md)
- [System Architecture](system/architecture.md)

## Operational Knowledge

- [Error Knowledge Base](../agent/error-kb.md)
